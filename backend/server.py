...
async def ideas_generate(data: IdeaRequest, user=Depends(get_current_user)):
    ...
    prompt = f"""Brainstorm {count} fresh, click-worthy YouTube video ideas for the niche: {data.niche}.
...
"""
    ...
    res = await db.history.insert_one(doc)
    return {"id": str(res.inserted_id), "ideas": parsed["ideas"]}
...
async def thumbnail_generate(data: ThumbnailRequest, user=Depends(get_current_user)):
    ...
    try:
        text, images = await chat.send_message_multimodal_response(UserMessage(text=final_prompt))
    except Exception as e:
